...
app.get('/', (req, res) => {
  res.json({
    message: 'ðŸš€ Toji Ultimate Bomber API is Running!',
    version: '1.0.0',
    developer: 'Toji',
    endpoints: {
...
app.get('/bomb', async (req, res) => {
  ...
  try {
    ...
    const responseData = {
      ...
      bombing_report: {
        call_bombing: callSuccess > 0 ? `âœ… Activated (${callSuccess} calls)` : "âŒ Failed",
        whatsapp_bombing: whatsappSuccess > 0 ? `âœ… Activated (${whatsappSuccess} messages)` : "âŒ Failed",
        sms_bombing: smsSuccess > 0 ? `âœ… Activated (${smsSuccess} SMS)` : "âŒ Failed"
      },
      detailed_results: results,
      developer: "TOJI",
      powered_by: "TOJI Ultimate Bombing API",
...
// Start server
app.listen(PORT, () => {
  console.log(`ðŸš€ TOJI Ultimate Bomber API running on port ${PORT}`);
  console.log(`ðŸ“ž Total APIs: ${BOMBING_APIS.length}`);
  console.log(`ðŸ”‘ API Keys: ${YOUR_API_KEYS.join(', ')}`);
